...
So for this problem, run this file; runP3.py
...
from P3classes import ActorCritic
from P3classes import Memory
from P3classes import PPO
...
def main():
    ...
    ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_BEFORE_stoch_randseed{}.mp4',
    ...
    ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_BEFORE_det_randseed{}.mp4',
                                          trial_num=trial_num, random_seed=random_seed, environment=environment,
                                          max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if episode_mean_reward > vid_threshold:
            ...
            plt.legend()
            plt.savefig(
                './pytorch_vids_cont3/p3CONT_BEST_{}_randseed{}_performancegraph.png'.format(trial_num, random_seed))
            plt.close()
            ...
            plt.legend()
            plt.savefig('./pytorch_vids_cont3/p3CONT_BEST_{}_randseed{}_smoothedperformancegraph.png'.format(trial_num,
                                                                                                             random_seed))
            plt.close()
            ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_BEST_stoch_randseed{}.mp4',
                                               trial_num=trial_num, random_seed=random_seed, environment=environment,
                                               max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                               std_scale=std_scale)

            ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_BEST_det_randseed{}.mp4',
                                                  trial_num=trial_num, random_seed=random_seed, environment=environment,
                                                  max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                                  std_scale=std_scale)
        ...
        if i_episode > 4:
            target = np.mean(mean_rewards[-5:])
            running_avgs.append(target)

        if i_episode % 23 == 0:
            ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_DURING_stoch_randseed{}.mp4',
                                               trial_num=trial_num, random_seed=random_seed, environment=environment,
                                               max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                               std_scale=std_scale)
    ...
    plt.title("Performance")
    plt.legend()
    plt.savefig('./pytorch_vids_cont3/p3CONT_{}_randseed{}_performancegraph.png'.format(trial_num, random_seed))
    plt.close()
    ...
    plt.title("Averaged Performance")
    plt.legend()
    plt.savefig('./pytorch_vids_cont3/p3CONT_{}_randseed{}_smoothedperformancegraph.png'.format(trial_num, random_seed))
    plt.close()

    # Film 'AFTER'' stochastic video
    ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_AFTER_stoch_randseed{}.mp4',
                                       trial_num=trial_num, random_seed=random_seed, environment=environment,
                                       max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)

    # Film 'AFTER' deterministic video
    ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont3/p3CONT_{}_AFTER_det_randseed{}.mp4',
                                          trial_num=trial_num, random_seed=random_seed, environment=environment,
                                          max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)
