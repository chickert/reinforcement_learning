"""
This file is associated with P3
"""
import time
...
class PusherEnv:
	...
	def compute_reward_push(self, state):
		state_grip = np.array(state[:3])
		state_obj = np.array(state[3:])

		puck_penalty = 0
		if float(state_obj[2]) < 0.95:
			puck_penalty = -1

		ltwo_grip_obj = np.linalg.norm(state_grip - state_obj, 2)
		# target1 = 2*np.exp(-(ltwo_grip_obj / (0.1**2))) / 0.1

		ltwo_obj_goal = np.linalg.norm(state_obj - self.goal, 2)
		# target2 = 3*np.exp(-(ltwo_obj_goal / (0.45 ** 2))) / 0.45

		target = np.exp(-4*(ltwo_grip_obj) - 0.5*(ltwo_obj_goal))

		# reward = -ltwo_grip_obj - ltwo_obj_goal + puck_penalty
		# target1 + target2 + puck_penalty

		# print("Target1 grip-obj", target1)
		# print("target2 obj-goal", target2)

		# reward = - ltwo_obj_goal + puck_penalty + target2 - target1

		reward = 6 * target + puck_penalty
