"""
This is the file associated with P1
"""
import time
...
class ReacherEnv:
	def __init__(self, action_repeat=30, render=False):
	...
	def compute_reward_reach(self, state):
		# Current best
		l_two = np.linalg.norm(self.goal - state, 2)

		reward = -l_two
		# Gaussian
		# l_two = np.linalg.norm(state - self.goal, 2)
		# reward = np.exp(-(l_two / (0.7**2))) / 0.7

		# Double Gauss
		# l_two = np.linalg.norm(state - self.goal, 2)
		# target1 = np.exp(-(l_two / (0.7**2))) / 0.7
		# target2 = np.exp(-(l_two / (1.**2))) / 1.
		# reward = target1 + target2
