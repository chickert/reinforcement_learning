...
So for this problem, run this file: runP2.py
...
from P2classes import ActorCritic
from P2classes import Memory
from P2classes import PPO
...
def main():
    ############## Hyperparamters ####################
    environment = ReacherWallEnv()
    state_dim = environment.observation_space.shape[0]
    action_dim = 2
    solved_reward = 10000000  # stop training if mean batch reward > solved_reward
    vid_threshold = 10000
    ...
    eps_clip = 0.2  # clip prameter for PPO
    random_seed = 303
    entropy_beta = 0.05
    critic_coef = 1
    std_scale = 0.5
    ...
    ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_BEFORE_stoch_randseed{}.mp4',
    ...
    ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_BEFORE_det_randseed{}.mp4',
                                          trial_num=trial_num, random_seed=random_seed, environment=environment,
                                          max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if episode_mean_reward > vid_threshold:
            ...
            plt.legend()
            plt.savefig(
                './pytorch_vids_cont2/p2CONT_BEST_{}_randseed{}_performancegraph.png'.format(trial_num, random_seed))
            plt.close()
            ...
            plt.legend()
            plt.savefig('./pytorch_vids_cont2/p2CONT_BEST_{}_randseed{}_smoothedperformancegraph.png'.format(trial_num,
                                                                                                             random_seed))
            plt.close()
            ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_BEST_stoch_randseed{}.mp4',
                                               trial_num=trial_num, random_seed=random_seed, environment=environment,
                                               max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                               std_scale=std_scale)

            ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_BEST_det_randseed{}.mp4',
                                                  trial_num=trial_num, random_seed=random_seed, environment=environment,
                                                  max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                                  std_scale=std_scale)
        ...
        if i_episode % 23 == 0:
            ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_DURING_stoch_randseed{}.mp4',
                                               trial_num=trial_num, random_seed=random_seed, environment=environment,
                                               max_timesteps=max_timesteps, ppo=ppo, memory=memory,
                                               std_scale=std_scale)
    ...
    plt.title("Performance")
    plt.legend()
    plt.savefig('./pytorch_vids_cont2/p2CONT_{}_randseed{}_performancegraph.png'.format(trial_num, random_seed))
    plt.close()
    ...
    plt.title("Averaged Performance")
    plt.legend()
    plt.savefig('./pytorch_vids_cont2/p2CONT_{}_randseed{}_smoothedperformancegraph.png'.format(trial_num, random_seed))
    plt.close()

    # Film 'AFTER'' stochastic video
    ppo.policy_old.film_stochastic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_AFTER_stoch_randseed{}.mp4',
                                       trial_num=trial_num, random_seed=random_seed, environment=environment,
                                       max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)

    # Film 'AFTER' deterministic video
    ppo.policy_old.film_deterministic_vid(filepath='./pytorch_vids_cont2/p2CONT_{}_AFTER_det_randseed{}.mp4',
                                          trial_num=trial_num, random_seed=random_seed, environment=environment,
                                          max_timesteps=max_timesteps, ppo=ppo, memory=memory, std_scale=std_scale)
